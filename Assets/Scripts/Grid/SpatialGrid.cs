...
public class SpatialGrid : MonoBehaviour
{
    ...
    private Dictionary<PathNode, Tuple<int, int>> lastPositions;
    //los "contenedores"
    private HashSet<PathNode>[,] buckets;
    ...
    readonly public PathNode[] Empty = new PathNode[0];
    #endregion

    #region FUNCIONES
    private void Start()
    {
        lastPositions = new Dictionary<PathNode, Tuple<int, int>>();
        buckets = new HashSet<PathNode>[width, height];

        for (int i = 0; i < width; i++)
            for (int j = 0; j < height; j++)
                buckets[i, j] = new HashSet<PathNode>();
    ...
    IEnumerator GetNodes()
    {
        ...
        var ents = RecursiveWalker(transform)
            .Select(x => x.GetComponent<PathNode>()).Where(x=>!x.isBlocked)
            .Where(x => x != null);
    ...
    public void ResetNodes()
    {
        for (int i = 0; i < width; i++)
            for (int j = 0; j < height; j++)
            {
                foreach (var item in buckets[i,j])
                {
                    item.Reset();
                }
            }
    ...
    public List<PathNode> GetPosibleTargets(Vector3 pos)
    {
        if (buckets != null)
        {
            var newPos = GetPositionInGrid(pos);
            return buckets[newPos.Item1, newPos.Item2].ToList(); 
        }
        return null;
    }

    public void UpdateEntity(PathNode entity)
    {
        if(entity==null)
            Debug.Log("...");
    ...
    public IEnumerable<PathNode> Query(Vector3 aabbFrom, Vector3 aabbTo, Func<Vector3, bool> filterByPosition)
    {
    ...
    //void OnDestroy()
    //{
    //    var ents = RecursiveWalker(transform).Select(x => x.GetComponent<PathNode>()).Where(x => x != null);
    //}
    ...
    public bool activatedGrid = true;
    public bool activatedGridLines = true;
    ...
    private void OnDrawGizmos()
    {
        ...
        if (!activatedGrid)
        {
            IEnumerable<PathNode> allElems = Enumerable.Empty<PathNode>();
